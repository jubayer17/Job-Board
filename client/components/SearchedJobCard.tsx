...
import { MapPinIcon, ClockIcon, CurrencyDollarIcon, BriefcaseIcon, ArrowRightIcon } from '@heroicons/react/24/outline';
...
export default function SearchedJobCard({ job }: { job: Job }) {
    // Theme classes configuration for Cyan variant
    const theme = {
        border: 'hover:border-cyan-600',
        text: 'group-hover:text-cyan-600',
        bg: 'bg-cyan-600',
        logoBorder: 'group-hover:border-cyan-600'
    };

    return (
        <div className={`group relative bg-white border border-gray-200 p-8 hover:bg-gray-50 transition-all duration-300 hover:shadow-lg ${theme.border} overflow-hidden rounded-none`}>
            {/* Top Border */}
            ...
            <div className="flex justify-between items-start mb-6 mt-4">
                <div className={`h-14 w-14 bg-white border border-gray-200 flex items-center justify-center font-bold text-xl text-gray-900 shadow-sm ${theme.logoBorder} transition-colors overflow-hidden rounded-none`}>
                    {job.logo?.startsWith('http') || job.logo?.startsWith('/') ?
                        <img src={job.logo} alt={job.company} className="w-full h-full object-cover" /> : <span className="text-2xl">🏢</span>}
                </div>
                <span className="px-2.5 py-1 border border-gray-200 text-xs font-bold text-gray-600 uppercase tracking-wide bg-gray-50 rounded-none">
                    {job.type}
                </span>
            </div>
            ...
            <div className="flex flex-wrap gap-2 mb-8">
                {job.tags.slice(0, 3).map(tag => (
                    <span key={tag} className="px-2 py-1 bg-white text-[10px] font-bold text-gray-500 uppercase tracking-wider border border-gray-200 group-hover:border-gray-300 rounded-none">
                        {tag}
                    </span>
                ))}
            </div>
